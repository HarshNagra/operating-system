...
void startProduction(){
  ...
  int total_rob = (num_typeA + num_typeB + num_typeC);
  int max = ((num_typeA + num_typeB + num_typeC) - num_spaces);
  if (num_spaces < total_rob){
    for (int i = 0; i < max; i++) {
      if (num_typeC > 0) num_typeC = num_typeC - 1;
      else if (num_typeB > 0) num_typeB = num_typeB - 1;
      else if (num_typeA > 0) num_typeA = num_typeA - 1;
      else break;
    }
  }
  ...
  sem_init(&task->makingBody, 0, 0);
  sem_init(&task->makingCar, 0, 0);

  sem_init(&task->skeleton, 0, 1); //1
  sem_init(&task->chassis, 0, 1); //1
  sem_init(&task->engine, 0, 1); //1

  sem_init(&task->battery, 0, 1); //1
  sem_init(&task->tire, 0, 4); //4
  sem_init(&task->window, 0, 7); //7

  sem_init(&task->body, 0, 0); //0
  sem_init(&task->car, 0, 1); //1
...
int main(int argc, char *argv[]) {
  ...
  printf("Name: Harsh Nagra\t UID: 3035437707\n"); 
  /* Initialization */
